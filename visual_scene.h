...
namespace octet { namespace scene {
  ...
  public:
    ...
    /// set up OpenGL state
    void begin_render(int vx, int vy, vec4_in clear_color=vec4(0.1f, 0.1f, 0.1f, 1.0f)) {
    ...
    void render(float aspect_ratio,int _cam=0) {
      if (camera_instances.size() != 0) {
        camera_instance *cam = camera_instances[_cam];
        render_impl(*object_shader, *skin_shader, *cam, aspect_ratio);
      }
    }
